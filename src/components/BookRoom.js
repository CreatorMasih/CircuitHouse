...
//     const apiUrl = 'https://script.google.com/macros/s/AKfycbzLzz23yX3PsMRz_VtMLSlW0tuKorS9j1cSPMWA_LOKQP_68ebYzKdKErQ20Ci7MdH7/exec'; // Replace with your API URL
...
const BookRoom = () => {
  ...
  useEffect(() => {
    // Fetch the room data from API using the Room Availability Check URL
    const fetchRoomData = async () => {
      try {
        const response = await fetch(
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "name") {
      ...
      setFormData({ ...formData, [name]: guests.toString() });
    } else if (name === "numberOfRooms") {
      let rooms = parseInt(value, 10);
      let maxRooms = formData.location === 'Dantewada' ? 6 : 2;
      rooms = rooms < 1 ? 1 : rooms > maxRooms ? maxRooms : rooms;
      setFormData({ ...formData, [name]: rooms.toString() });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    // Room Booking URL
    const apiUrl = 'https://script.google.com/macros/s/AKfycbzsd7W4vDo4C8RHC9Rt85ix4xf14eqm6AJj2C0FQbwP1SdJe4fdB2GXVEkya2ZATnyx/exec';
    ...
    try {
      ...
      setTimeout(() => {
        fetch('https://script.google.com/macros/s/AKfycbykXHy-Z9XF3ZBKg_7-al0bYYBkhqP_-A3KMwEYleL0A6M8t0LSHzEgqYY7zP_ZTkC5/exec') // Room Availability Check URL
          .then(response => response.json())
          .then(data => {
            console.log('Success:', data);
            setRoomData(data);
